...
if(! $con)
{
		...
		$all_hair = mysqli_query($con,"SELECT * FROM hair_category");
...
if(! $con)
{
		...
		$sqlloc = mysqli_query($con,$locquery);
		$locnum = mysqli_num_rows($sqlloc);
		return $locnum;
	}//end of numloc
...
if(! $con)
{
		...
		$sql = mysqli_query($con,$hair);
		$hairnum = mysqli_num_rows($sql);
		return $hairnum;
	}//end of numhair
...
if(! $con)
{
		...
		$locquery = "SELECT Studio_Name FROM all_studios WHERE region = '$city'";
		$queryloc = mysqli_query($con,$locquery);
		$locnum = mysqli_num_rows($queryloc);
		...
		while($row = mysqli_fetch_assoc($queryloc)){
...
if(! $con)
{
		...
		$hair = "SELECT DISTINCT all_studios.Studio_Name 
				...
				AND pricing.Style_ID = hair_styles.Style_ID
				AND hair_styles.Category_ID = '$hair_cate'"; //Note to Amy, Please define the checkbox in the following manner <select name = "hair_cate">
		$queryhair = mysqli_query($con,$hair);
		$hairnum = mysqli_num_rows($queryhair);
		$hairresults = array();
		...
		while($row = mysqli_fetch_array($queryhair, MYSQLI_ASSOC)){
			$hairresults[] = $row;
			//echo $row['Studio_Name']. '<br/>_________________________ <br/>' ;
		}//end of while loop
		return $hairresults;
