<?php
	...
	function minprice(){
		$min_price = mysql_query("SELECT DISTINCT Min_Price FROM pricing2 order by Min_Price ASC");
	...
	function maxprice(){
		$max_price = mysql_query("SELECT DISTINCT Max_Price FROM pricing2 ORDER BY Max_Price ASC");
	...
	function priceSearch($min, $max, $hair){
		...
		if(isset($_POST['shair'])){
			$sql = "SELECT DISTINCT all_studios.Studio_Name
				FROM all_studios, pricing2, hair_styles
				WHERE all_studios.Studio_ID = pricing2.Studio_ID 
				AND pricing2.Style_ID = '$hair_style'    
				AND pricing2.Min_Price >= '$min_price'
				AND pricing2.Max_Price <= '$max_price'"; 
